...
Aski — a Swift package that converts images to ASCII art. The current default uses 60D log-polar matching and OKLab color processing. Targets iOS 18+, macOS 15+, visionOS 2+. Strict Swift 6 concurrency.
...
- **No public-API stability contract.** Source-breaking changes are acceptable when they materially improve output quality, app fit, performance, velocity, or maintainability. Aski is the foundational layer for ASCII art applications, not a public SDK.
...
- **Release hygiene before tags.** Before pushing any future `v*` tag, commit the matching `CHANGELOG.md` entry and `docs/release-notes/<tag>.md`.
...
Canonical commands live in `justfile`; `Makefile` forwards the same target names for `make`-based environments.
...
```bash
swift run aski [render] <image> --columns 80 [--charset blocks] [--background clear] [--output <path>] [--render-png <path> [--width <px>] [--font-size <n>]] \
...
swift run aski inspect <image> --columns 80 [--format text|json] [--output <path>]       # resolved conversion metadata; JSON schema v1 under docs/assets/schemas/
swift run AskiTileMatrix <image> <out> --columns 64 --scale 12                       # tile-grid render matrix
swift run aski lab color <subcommand> --output-dir <dir>                               # color-pipeline research (many subcommands; see --help)
swift run -c release aski lab color render-matcher-challenge --output-dir <dir>         # ASKI-69 one-shot fixed-footprint matcher challenge
swift run aski lab motion --output-dir <dir> --preset all --columns 80 --fps 12        # animation / motion presets, GIF export
swift run aski lab video --input <clip.mp4|gif> --output-dir <dir> --columns 80        # MP4/GIF decode → ASCII → re-encode (+ quality/effect levers; see --help)
swift run aski lab accessibility audit --output-dir <dir> --columns 80                # palette / grid CVD distinguishability audit
swift run aski lab decolor check --output-dir <dir> --columns 80                       # composited-cell perceptual oracle — FAIL-FAST (nonzero exit == KILL)
swift run aski lab hdr check --output-dir <dir> --columns 80                           # HDR/EDR emissive-spike gates — FAIL-FAST (nonzero exit == KILL)
swift run aski lab preset ab --input <portrait> --output-dir <dir>                     # ASTSK-47 charset×columns A/B → candidate PNGs + contact sheet + manifest (feed it REAL faces)
swift run -c release aski lab preset probe-stimuli --source-manifest <private.json> --output-dir <public-dir> --key-output <private-key.json>  # ASKI-73 blinded three-arm stills + center reveal
swift run aski lab preset probe-score --stimuli-manifest <manifest.json> --key <private-key.json> --session-one <private.csv> --repeat-log <private.csv> --output-dir <aggregate-dir>  # validates and scores frozen product gates
```

The historical `Aski*Lab` product names remain thin compatibility shims over the surviving lab commands. Removed experiment runners remain in Git history, with their invocations captured by the dated notes and artifacts. Use `aski lab …` for new commands and automation.
...
- **Docs discovery hub (know-X → read-Y, tool inventory)** → [docs/README.md](docs/README.md)
