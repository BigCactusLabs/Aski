...
@Suite struct ASCIIConverterMaskTests {
    ...
    @Test func maskCoverageAndGridPolicyRoundTrip() {
        let image = TestImages.horizontalGradient(width: 120, height: 60)
        let mask = Self.horizontalFourStepMask()
        let fallback = CGColor(red: 1, green: 0, blue: 0, alpha: 1)
        let ground = CGColor(red: 0, green: 0, blue: 0, alpha: 0.75)
        ...
        #expect(grid.maskUsesHardEdges)
        #expect(grid.maskFallback != nil)
        #expect(grid.maskGroundColor?.alpha == 0.75)
    ...
    @Test func invertedHardMaskFlipsThresholdedCoverage() {
        let image = TestImages.horizontalGradient(width: 120, height: 60)
        let grid = DefaultConverter().convert(
            image,
            columns: 4,
        ...
        for row in grid.cells {
            #expect(row[0].coverage == 1)
            #expect(row[1].coverage == 1)
            #expect(row[2].coverage == 0)
            #expect(row[3].coverage == 0)
        }
    }
